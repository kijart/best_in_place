...
BestInPlaceEditor.prototype = {
  ...
  updateIfConfirm: function() {
    if(this.confirmUpdate) {
      if(!confirm(this.confirmUpdate)) {
        this.abort();
      } else {
        this.update();
      }
  ...
  initOptions : function() {
    ...
    self.element.parents().each(function(){
      ...
      self.confirmUpdate    = self.confirmUpdate      || $parent.attr("data-confirm-update");
    });
    ...
    self.confirmUpdate    = self.element.attr("data-confirm-update")     || self.confirmUpdate;
  ...
  "input" : {
    ...
    inputBlurHandler : function(event) {
      if (event.data.editor.okButton) {
        ...
        if (event.data.editor.cancelButton) {
          event.data.editor.blurTimer = setTimeout(function () {
            if (!event.data.editor.userClicked) {
              event.data.editor.updateIfConfirm();
            }
          }, 500);
        } else {
          event.data.editor.updateIfConfirm();
    ...
    keyupHandler : function(event) {
      if (event.keyCode == 27) {
        ...
        event.data.editor.element.find('input').blur();
      }
    }
  },
